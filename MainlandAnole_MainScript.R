...
a <- ggplot.pca(pca = phylopca, axis1 = 1, axis2 =2, species = species, 
                groups = eco, labels = FALSE, 
                region = NewData[species,"Region"])
b <- ggplot.pca(pca = phylopca, axis1 = 1, axis2 =3, species = species, 
                groups = eco, labels = FALSE, 
                region = NewData[species,"Region"])
c <- ggplot.pca(pca = phylopca, axis1 = 4, axis2 =3, species = species, 
                groups = eco, labels = FALSE, 
                region = NewData[species,"Region"]) + 
  scale_y_continuous(labels = scales::number_format(accuracy = 0.1))
d <- ggplot.pca(pca = phylopca, axis1 = 4, axis2 =5, species = species, 
                groups = eco, labels = FALSE, 
                region = NewData[species,"Region"])
...
tapply(criteria.lda.trim$Pred.Eco[which(criteria.lda.trim$Ecomorph == "Unknown")],
       criteria.lda.trim$Pred.Eco[which(criteria.lda.trim$Ecomorph == "Unknown")],length)


# Euclidean Criteria w/Caribbean ecomrophs --------------------------------

# predict ecomorphs based on certain criteria

predicted <- predict.class(scores = phylopca$S[tree$tip.label,1:6], species = tree$tip.label, 
...
# Compile Assignments w/ Caribbean ecomorphs ------------------------------
